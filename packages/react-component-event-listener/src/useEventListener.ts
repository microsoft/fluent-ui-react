...
const useEventListener = <T extends EventTypes>(options: EventListenerOptions<T>): void => {
  ...
  React.useEffect(() => {
    ...
    if (isActionSupported(element, 'addEventListener')) {
      ...
      throw new Error(
        '@stardust-ui/react-component-event-listener: Passed `element` is not valid or does not support `addEventListener()` method.',
      )
    }
    ...
    return () => {
      if (isActionSupported(element, 'removeEventListener')) {
        ...
        throw new Error(
          '@stardust-ui/react-component-event-listener: Passed `element` is not valid or does not support `removeEventListener()` method.',
        )
      }
    }
