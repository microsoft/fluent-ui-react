...
describe('Focus Container', () => {
  let focusContainer: ContainerFocusHandler<
    ShorthandValue,
    FocusContainerProps<ShorthandValue>,
    FocusContainerState
  >

  const items = [{ title: 'First Item' }, { title: 'Second Item' }, { title: 'Third Item' }]
  const props: FocusContainerProps<ShorthandValue> = {
    items,
  }
  ...
  let setStateMock: SetStateDelegate<FocusContainerProps<ShorthandValue>, FocusContainerState>
