import { Flex, Grid, Segment } from '@stardust-ui/react'
...
`Flex` is for columns _or_ rows. This means that if you are laying out items in one direction (for example buttons inside a header), then you should use `Flex`.
...
More examples are available [Grid component docs](/components/grid).
