...
The theming mechanism as part of Fluent UI should be flexible and provide support for CSSinJS theme, and all existing customization scenarios. It must also allow users to opt-out if the theming approach does not meet their needs. This means that we need to have support for both CSS stylesheets and CSSinJS.
...
CSSinJS support that will not tie the users to just one CSS in JS framework. The users should be able to define their styles and specify their engine that will be aligned to some defined interface.
...
It is important to allow the users to specify the RTL/LTR mode for the entire application once, but if necessary they can specify it only in a subtree of the application (for example one ChatMessage)
...
The slots prop must be accessible in several means to ensure that a "recursive component" can be accurately styled.
...
This is inspired by the [React Semantic UI's className builders](https://github.com/Semantic-Org/Semantic-UI-React/blob/master/src/lib/classNameBuilders.js)

Here is an example of how the mappings for a FluentButton component can look like:

```
const fluentButtonMappings = props => {
  const { size, circular, attached, corner } = props
  return cx(
    size, // the value (small, medium, big)
    useKeyOnly(circular, 'circular'),
    useKeyOrValueAndKey(attached, 'attached'), // true or the value of the attached prop
    useValueAndKey(corner, 'corner'), // corner="left" will create the classNames `corner left`
  )
}

const FluentButton = compose(Button, {
  displayName: 'FluentButton',
  mapPropsToClasses: fluentButtonMappings,
})
```
...
I will describe one proposal of how the theme object can look like:
