  ...
  render() {
    return (
      <DocPage title="Shorthand Props">
        <p>
          It is quite common for Stardust component to have "slots" which accept shorthand values.
          For example, {code('Button')} component has an {code('icon')} slot which value defines the
        ...
        <p>
          Each component's slot has associated default element type. For example, by default there
          is {code('<Icon />')} element rendered for {code('Button')}'s {code('icon')} slot. It is
          possible to customize props of this default element by providing props object as shorthand
          value:
        </p>
        ...
        <p>
          There are cases where it might be necessary to customize element tree that will be
          rendered as a slot's value. Returning to {code('Button')} example, we might want to render{' '}
          {code('<i />')} instead of default {code('<Icon />')} element. In that case necessary
          element might be directly provided as shorthand value:
        </p>
        {codeExample([`<Button icon={<i class='my-icon'></i>} />`])}

        <blockquote>
          <strong>
            There is a very important caveat here, though: whenever React Element is directly used
            as a shorthand value, it becomes client's responsibility to handle some aspects that
            Stardust was originally responsible for (such as {code('styles')} or{' '}
            {code('accessibility')}).
          </strong>{' '}
        ...
        <p>
          ...
          slot, while wanting to preserve calculated styles and accessibility aspects for it. In
          that case {code('render')} callback of function shorthand value should be used.
        </p>

        {/* SHORTHAND AS A FUNCTION */}
        {header('Shorthand value as Function')}
        <p>
          Providing function as a shorthand value is the most involving but, at the same time, the
          most powerful option for customizing component's slot. The only requirements for this
          function are:
        </p>
        ...
        <p>
          However, if it will be just about that, this function form wouldn't introduce any
          additional value. So, lets take a look on the scenarios where it is really helpful.
        </p>

        {subheader(`'Render' callback argument`)}
        <p>
          When function is provided as a shorthand value, there is a {code('render')} callback
          provided as its argument. This {code('render')} argument is a function that 'knows' how to
          render all the previously considered shorthand types - i.e. primitive values, objects,
          React Elements.
        </p>
        ...
        <p>
          This 'render callback' arg makes it possible for the client to tackle scenarios where
          slot's data may be fetched asynchronously.
        </p>

        <p>
          Consider the following scenario: suppose we need to render {code('Button')}'s icon, but
          the problem is that icon's name is fetched from some remote source. Thus, we cannot do it
          as simply as that, as icon's name is not known at the moment {code('Button')} is rendered:
        </p>
        ...
        <p>
          ...
          abstraction - and it is possible to 'tell' this component what should be rendered while
          data is loading, as well as what should be rendered when data is fetched.
        </p>
        {codeExample([
          `<AsyncData`,
        ...
        <p>
          Lets suppose that we want to render 'loading' icon before data is fetched, and switch it
          with the icon which {code('name')} will be provided by fetched {code('data')} object.
          Also, note that we have {code('render')} arg that knows <em>how</em> to render{' '}
          {code('Button')}'s icons.
        </p>
        ...
        <p>
          Note that this example can be further simplified, as {code('render')} can be used to
          render 'loading' icon as well:
        </p>
        {codeExample([
          `<Button`,
          `  icon={render => <AsyncData`,
        ...
        <p>
          There is another use case that {code('render')} callback arg is very useful for - this is
          the case where custom element's tree should be rendered for the slot, but, at the same
          time, all the evaluated styles and accessibility behaviors should be preserved for
          rendered custom element.
        </p>
