...
export default () => (
  <DocPage title="Accessibility in Stardust">
    ...
    <p>
      Besides component level accessibility there are application / page level considerations,
    ...
    <p>
      This is a simple example, clearly here the intent is to display a button with an icon labelled
      'Download'. HTML representation is semantically correct and specifies essential
    ...
    <p>
      Although we highly recommend using semantically correct HTML elements, it is possible to
      render, for example, {code('<Button>')}
      component as {code('<div>')} and remain semantic by provided out of the box {code('role')}{' '}
      attribute.
    </p>
    ...
    <ul>
      <li>
        Preserve DOM order and put elements in their correct position - use css to style appearance,
        not position
      </li>
      <li>
        Use appropriate semantic elements (for example {code('<table>')} for a table, {code('ul')}{' '}
        and {code('li')}
        for a list)
      </li>
    ...
    <p>
      ...
      right and choosing the correct behaviors. For example
    </p>
    <p>
    ...
    <p>
      ...
      with this behavior as they, for example, move between fields in form using {code('Tab')}. This
      is provided by the browser and for simple applications may be sufficient.
    </p>
    <p>
    ...
    <p>
      ...
      the reader without any additional work. However, if you do need to customize behavior, then
    ...
    <p>
      Out of scope for now. Can be used{' '}
      {link('react-aria-live', 'https://github.com/AlmeroSteyn/react-aria-live')}
