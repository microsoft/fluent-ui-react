...
export default () => (
  <DocPage title="Accessibility in Stardust">
    ...
    <ul>
      ...
      <li>
        Use appropriate elements (for example &lt;table&gt; for a table, &lt;ul&gt; and &lt;li&gt;
        for a list)
      </li>
    ...
    <p>
      ...
      one approach is for focus to first land on 'Stardust UI', and then when you press tab to move
      to '...'. Alternatively, this could be regarded as one control and you would navigate within
      the control using arrow keys / enter / escape and pressing 'tab' would move you to the next
      area. When you land on the control in both cases the screen reader needs to give context and
    ...
    <p>
      could be implemented as separate controls, so you would navigate between them with tab. Or
      they could be grouped together to behave as a toolbar. The first approach has advantages that
      blind users would have awareness of all the controls available to them, but at the expense of
      having to press tab multiple times to move around the application.
    </p>
    <p>
      ...
      - it is still costly in terms of developer time to correctly make these types of changes
      later.
    </p>
    <p>
    ...
    <p>
      ...
      browser and for simple applications may be sufficient.
    </p>
    <p>
      ...
      process of moving between them with tab becomes un-usable. This can be solved by breaking the
      application up into 'Zones', and the user navigates with the tab key between the zones, and
      between the actionable elements with navigation keys. Stardust uses Focus Zones both within
      it's own library components and as a component that can be added by the user.
    </p>
    ...
    <p>
      ...
      use the Tab key to navigate between higher level components (for example tool bars, menus,
      lists) and use arrow key navigation within these higher level components (buttons in a
    ...
    <ul>
      <li>
        Inside of Stardust components - for example Menu component will use FocusZone internally to
        add arrow key navigation. This is implicit to the developer.
      </li>
      <li>
        Wrapping (Stardust) components - For example it is possible to combine Accordion with List
        and wrap them in a FocusZone to get the chat list experience:
      </li>
    </ul>
    <CodeSnippet
      label="App.jsx"
    ...
    />
    <p>FocusZone operates based on DOM structure to:</p>
    <ul>
    ...
    <p>
      ...
      then might want to interact with the message itself (press like button, edit the message, open
      collapsed message). Stardust will enable this multi-level navigation - user will be able to
      press Enter and Escape to go in and out of the subcomponent navigation.
    </p>
    ...
    <p>
      ...
      the element that currently has focus clearly visible, so the user can see where they are on
    ...
    <p>
      ...
      navigation. They essentially add aria roles, aria attributes and even handlers. The idea is to
      compose visual components and apply a behaviour on top of them to achieve desired keyboard or
      screen reader navigation.
    </p>
    <p>
      Each relevant component comes with its default accessibility behaviour. For some components
      there will be additional behaviours to choose from. In addition to that, user can create his
      own behaviours and use them instead of the standard ones.
    </p>
    <p>
    ...
    />
    <p>
      Default accessibility behaviour for Menu component is MenuBehavior (MenuItemBehavior for the
      menu items). These behaviours add appropriate ARIA roles by default:
    </p>
    <p>Rendered HTML:</p>
    <CodeSnippet
      label="App.jsx"
    ...
    <p>
      NOTE: If ChildAPI is used (rather than the shortcut API), Stardust does not modify the
      children components in any way. For example, if content of Menu.Item is provided as a child
      component, no behaviours are applied on these children. Example:
    </p>
    <CodeSnippet
    ...
    />
    <p>
      Notice that Stardust only applies role only on the &lt;li&gt; element which it rendered. The
      "Editorials" component that was passed to Stardust is not modified in any way and thus does
      not have the role applied.
    </p>
    ...
    <p>
      There are multiple ways (aria-label, aria-labelledby, title) to set the text that is announced
      by screen readers. Many times, the correct text will be read by the reader without any
      additional work. However, if you do need to customize behaviour, then aria attributes can be
      passed to components as properties.
    </p>
    <p>
    ...
    <ul>
      <li>
        Title attribute represents the text that is shown as a tooltip on hover, screen readers also
        read this text when the element is focused
      </li>
      <li>
        Aria-label allows the user to add custom text that the screen reader will read on focus.
        There might be scenarios where the information read by the screen reader needs to be
        different than the information that comes from the visual representation of the
        component/element
      </li>
      <li>
        Aria-labelledby is similar to aria-label, but is only a reference to a different element
        present in DOM.
      </li>
    </ul>
    <p>
