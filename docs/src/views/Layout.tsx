...
export default () => (
  <DocPage title="Layout Components">
    ...
    <ul>
      ...
      <li>
        {links.grid} - designed for two-dimensional layout, rows and columns at the same time.
      </li>
    </ul>
    <Header as="h3">Flex vs Grid</Header>
    <p>
      Sometimes it might not be evident if some particular case is a case of 1D (i.e. {code('Flex')}
      ) or 2D (i.e. {code('Grid')}) layout. As a result, quite often {code('Grid')} component is
      used at places where it is sufficient to use {code('Flex')}.
    </p>
    <p>Here is the question you might ask yourself to decide on that:</p>
    <ul>
      <li>
        do I only need to control the layout by row <strong>or</strong> column? – if yes, use{' '}
        {code('Flex')}.
      </li>
      <li>
        do I need to control the layout by row <strong>and</strong> column? – if yes, use{' '}
        {code('Grid')}.
      </li>
    </ul>
    ...
    <p>
      This description suggests that purpose of {code('Segment')} is not a layout. Thus, this
      component shouldn't be used for layout purposes, as this will break component's semantics -
    ...
    <blockquote>
      Use {code('Flex.Item')} component to wrap child elements only in case if their flex styles
      should be overriden.
    </blockquote>
    <p>
      There is no strict need to use {code('Flex.Item')} component as a direct child of{' '}
      {code('Flex')} - one may use {code('Flex.Item')} component only when it is necessary to tweak
      flex styles of individual child item.
    </p>
    <ExampleSnippet
      render={() => (
        <Flex gap="gap.small">
    ...
    <blockquote>
      Each {code('Flex.Item')} element being introduced without any props specified should be
    ...
    <p>
      Flex component was designed being able to address all flexbox layout scenarios with minimal
      amount of DOM elements rendered. In addition, there are props to address most common{' '}
      {link(
    ...
    <p>
      Let's consider one of the most representative examples - a navigation menu. Suppose that we
      want a menu bar with logo on the left side and set of nav links on the right.
    </p>
    <p>
      Quite often this is achieved by introducing a top-level flex container with two children:
      first is a logo, and the second one is another container ({code('Flex')}, renders to{' '}
      {code('<div />')}) with a group of buttons:
    </p>
    <ExampleSnippet
      render={() => (
        <Flex space="between">
    ...
    />
    <p>
      It turns out that this approach introduces unnecessary nesting level - it is possible to
      achieve the same goal by using just top-level container, with no buttons container:
    </p>
    <ExampleSnippet
      render={() => (
        <Flex gap="gap.small">
    ...
    <p>
      Note that{' '}
      <strong>{code('Flex.Item')} doesn't result in any additional DOM element rendered</strong> -
      its sole purpose is just to pass style props to its child.
    </p>
    <blockquote>
      Consider to review set of examples on the {links.flex} page - as there might be an example
