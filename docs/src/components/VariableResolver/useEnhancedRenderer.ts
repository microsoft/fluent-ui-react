...
const useEnhancedRenderRule = (
  renderer: Renderer,
): [Renderer['renderRule'], React.RefObject<ThemeComponentVariablesPrepared>] => {
  const variables = React.useRef<ThemeComponentVariablesPrepared>({})
    ...
    (rule, props) => {
      ...
      _.forEach(flattenStyles, styleValue => {
        if (typeof styleValue === 'string' && styleValue.startsWith('variable.')) {
          ...
          variables.current[componentName][variableName] = props.variables[variableName]
        }
      })
...
): [Renderer, React.RefObject<ThemeComponentVariablesPrepared>] => {
