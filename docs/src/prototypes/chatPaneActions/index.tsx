...
const actionMenu = (
  ...
  //       { key: 'more', icon: 'more', 'aria-label': 'More options', menu: ['Save','Edit', { content: 'Remove', menu: ['1', '2', '3']}] },
  //     ]
  //   }
  // }
  return render =>
...
const CustomMessage = (props: ChatMessageProps) => {
  ...
  const show = (es: EventSource, event: React.SyntheticEvent<HTMLElement, Event>) => {
    ...
    if (event && event.target && es !== 'mouse') {
      const targetElement = event.target as HTMLElement
      if (!(targetElement.getAttribute('aria-roledescription') === 'message')) {
